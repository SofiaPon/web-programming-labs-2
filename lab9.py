...
def main():
    ...
    if request.method == 'POST':
        if 'name' in request.form:  # Если форма с именем отправлена
            name = request.form.get('name')
            return redirect(url_for('lab9.age', name=name))  # Переход на страницу возраста
        else:  # Если это запрос на сброс данных
